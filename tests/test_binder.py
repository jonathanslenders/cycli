...
def test_curly_left(binder):
  curly_left(binder)
  binder.app.current_buffer.insert_text.assert_has_calls([call("{"), call("}", move_cursor=False)])


def test_curly_right_curly_current(binder):
  binder.app.current_buffer.document.current_char = "}"
  curly_right(binder)
  binder.app.current_buffer.cursor_right.assert_called_once_with()


def test_curly_right_not_curly_current(binder):
  binder.app.current_buffer.document.current_char = "n"
  curly_right(binder)
  binder.app.current_buffer.insert_text.assert_has_calls([call("}")])
...
def test_backspace_no_pattern(binder):
  binder.app.current_buffer.document.current_char = "n"
  backspace(binder)
  binder.app.current_buffer.delete_before_cursor.assert_called_once_with()
...
def test_backspace_with_pattern(char_before_cursor, current_char):
  ...
  binder.app.current_buffer.document.char_before_cursor = char_before_cursor
  binder.app.current_buffer.document.current_char = current_char

  backspace(binder)

  binder.app.current_buffer.cursor_right.assert_called_once_with()
  binder.app.current_buffer.delete_before_cursor.assert_called_once_with(2)
